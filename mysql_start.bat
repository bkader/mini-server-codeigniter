...
if NOT exist core\mysql\data\mysql_mini.pid goto :NOTSTARTED

rem ## It exists is it running
SET /P pid=<core\mysql\data\mysql_mini.pid
...
del core\mysql\data\mysql_mini.pid 
...
(set /p dummy=%Disk%) >core\mysql\data\drive.txt <nul

rem ## Start server
%Disk%:
:start core\mysql\bin\mysqld-opt.exe --defaults-file=/bin/my-small.cnf
start core\mysql\bin\mysqld-opt.exe
